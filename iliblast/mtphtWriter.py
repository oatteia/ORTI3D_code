...
class mtphtWriter:
    ...
    def writeFiles(self,opt):
        ...
        lexceptions=['dsp.1','dsp.3','dsp.4','dsp.5','btn.7','btn.8','btn.21','btn.22',
                'rct.1']
        ...
        for grp in self.core.getUsedModulesList('Mt3dms'):
            ...
            llist=Mkey.groups[grp];#print n1,name
            for ll in llist:
                cond=Mkey.lines[ll]['cond'];#print 'mtw 72',ll
    ...
    def getConcInit(self,typ,line,iper=0):
        ...
        if 'importUCN' in initChem['formula']:
            if 'All' in initChem['name']:
                a = initChem['formula'].split('tstep :')[1].split('\n')[0]
                tstep = int(a)
                ie = 0; #print tstep,ie
                for kw in shortn:
                    for e in dictE[kw]:
    ...
    def getRctParms(self,opt,line):
        ...
        if flag =='Pht3d': #writes values ofr each pht3d species
