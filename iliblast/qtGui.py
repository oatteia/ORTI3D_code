...
class Box:
    def __init__(self,Parameters,parent,gr,nb):
        ...
        self.hl.setMargin(0)
        dirutils = self.parent.plugin_dir+os.sep+'utils'
...
class Ui_Var(object):
    def setupUi(self, Var,gui,core):
        ...
        categories = ['Modflow series','Min3p','Fipy','Sutra']
        label = QLabel(self.gridLayoutWidget)
...
class Ui_Show(object):
    def setupUi(self,Show,gui,core):
        ...
        for ig in range(len(self.groups)): 
            ...
            self.dictBox[g] = qtBox(Show,self.gui.guiShow,names,g,pos)
            pos += len(names)*24+20
        Show.resize(200,560)
    ...
    def getNames(self,nameBox):
        ...
        return names
    def setNames(self,nameBox,names):
        combo = self.Show.findChild(QComboBox,nameBox) 
