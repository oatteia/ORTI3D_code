...
"""        
#/////////////////////////////////////////////////////////////////////
class wxVisualisation(FigureCanvasWxAgg):
...
#        pl.title(self.traduit(str(titre))+s[:9],fontsize=20)
    ...
    def initDomain(self):
        ...
        self.cnv.set_xlim(self.xlim)
        self.cnv.set_ylim(self.ylim)
        if self.core.Zblock==None: self.core.Zblock = makeZblock(self.core)
...
#        
    ...
    def createGrid(self,col=None,ori='Z',layer=0): 
        ...
        if self.mesh:# case irregular mesh (from matplotlib2dviewer)
            lines,opgeo = [],self.core.addin.opgeo
            xcoo = opgeo.elx
            ycoo = opgeo.ely
            for x,y in zip(xcoo,ycoo):
                lines.append(zip(x,y))
            self.Grid[0] = LineCollection(lines)
            self.Grid[1] = LineCollection(lines[:2])
            self.Triangles = opgeo.trg

        else: #rectangular cases
            xl,yl = getXYmeshSides(self.core,self.curOri,self.curLayer)
            dep=zip(ravel(xl[:-1,:]),ravel(yl[:-1,:]))
            arr=zip(ravel(xl[1:,:]),ravel(yl[1:,:]))
...
#        self.Map.set_visible(bool)
    ...
    def createImage(self,data):
        ...
        if self.mesh==False: # classical square grid
            image=pl.pcolormesh(X,Y,Z) #,norm='Normalize')
        else :
            pass
#            opgeo = self.core.addin.opgeo
#            x,y = opgeo.nodes[:,1],opgeo.nodes[:,2]
#            triang = mptri.Triangulation(x, y)
#            image=plt.tripcolor(triang,Z1,shading='gouraud')#PolyCollection(polys) #plt.tripcolor(triang, z, shading='gouraud', cmap=plt.cm.rainbow)
...
#            #image.set_edgecolor('none')
    ...
    def createVector(self,data):
        ...
        opgeo = self.core.addin.opgeo
        if self.mesh :
            X,Y = opgeo.nodes[:,1],opgeo.nodes[:,2]; #print X,Y,U,V
